...
<body>
            ...
            <div class="ml-auto flex items-center space-x-4">
                ...
                <?php if ($is_employee): ?>
                    <a href="/vacation_app/local/index.php?action=edit_profile" class="flex items-center">
                        ...
                        <span class="ml-2">Hallo, <?php echo htmlspecialchars($username); ?></span>
                    </a>
                <?php else: ?>
    ...
    <h2 class="flex justify-center my-8 text-4xl">Unternehmenskalender</h2>
    <div id='calendar'></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            //from vacationControler
            <?php foreach ($vacations as $vacation):?>

                events.push({
                    title: '<?php echo $vacation['username'] . " (" . $vacation['type_name'] . ")"; ?>' + '<?php echo !empty($vacation["half_day_period"]) ? " - " . $vacation["half_day_period"] : ""; ?>',
                    start: '<?php echo $vacation['start_date']; ?>',
                    //al "end" le debemos agregar un dia , ya que en el calendario solo se muestra hasta un dia antes.
                    end: '<?php echo date('Y-m-d', strtotime($vacation['end_date'] . ' +1 day')); ?>',
                    half_day_period: '<?php echo $vacation['half_day_period']; ?>]',
                    color:'<?php echo ($vacation["type_name"] === "Krank") ? "#FF0000" : "#3788d8"; ?>'

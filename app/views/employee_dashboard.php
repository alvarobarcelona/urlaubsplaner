...
// PUNTO 1: Obtener el estado de vacaciones del empleado
$employee = $vacationModel->getEmployeeVacationStatus($employee_id);
$total_days = $employee['total_vacation_days'];
$used_days = $employee['used_vacation_days'];
$pending_days = $total_days - $used_days;
$total_sick_days = $employee['sick_days'];
$total_special_holidays_days = $employee['special_holidays_days'];

// PUNTO 2: Obtener las próximas vacaciones aprobadas
$next_vacation = $vacationModel->getNextApprovedVacation($employee_id);

// PUNTO 3: Obtener el historial de solicitudes de vacaciones
...
<body>
    <nav>
        <ul>
            ...
            <li><a href="general_dashboard.php">Mein Kalender</a></li>
...
<!-- Próximas vacaciones aprobadas -->
        ...
        <?php if ($next_vacation): ?>
            <div class="vacation-dates">          
                <?php
                    ...
                    $next_vacation_start = DateTime::createFromFormat('Y-m-d', $next_vacation['start_date'])->format('d/m/Y');
                    $next_vacation_end = DateTime::createFromFormat('Y-m-d', $next_vacation['end_date'])->format('d/m/Y');           
                ?>
    ...
    <h3 style="margin-left: 20px;">Verlauf der Anträge:</h3>
    <table>
        <thead>
            <tr>
                <th>Startdatum</th>
            ...
            <?php if (!empty($vacation_history)): ?>
                ...
                <?php if ($vacation['status'] == 'Pending' || $vacation['status'] == 'Approved'): ?> 
                    <form action="/vacation_app/local/index.php?action=cancelVacation" method="post" style="display:inline;">
                        ...
                        <button type="submit" onclick="return confirm('Sind Sie sicher, dass Sie diese Bewerbung stornieren möchten?');">Cancelar</button>
                    </form>
                <?php else: ?>
                    <span>-</span>
                <?php endif; ?>
